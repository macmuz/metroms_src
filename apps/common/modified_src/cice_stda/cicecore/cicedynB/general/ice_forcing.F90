...
!
      ...
      integer (kind=int_kind) :: &
           ...
           oldfyear = 0   , & ! old forcing year (save between steps)
           oldrecd = 0    , & ! old baseline record number  (save between steps)
           oldrecnum4X = 0    !
...
!
      ...
      allocate ( &
          ...
          stray_data(nx_block,ny_block,2,max_blocks), &
           ...
           zeta_data(nx_block,ny_block,2,max_blocks), &
         frzmlt_data(nx_block,ny_block,2,max_blocks), &
...
! namelist input; initialize the atmospheric forcing data filenames.
    ...
    !-------------------------------------------------------------------
      ...
      if (trim(atm_data_type) == 'iopas') then
         call iopas_files(fyear)
      elseif (trim(atm_data_type) == 'ncar') then
...
!        call ocn_data_ncar_init_3D
      ...
      if (trim(ocn_data_type) == 'iopas') then
         call ocn_data_iopas_init
      endif
...
!MHRI: NOT IMPLEMENTED YET
      elseif (trim(ocn_data_type) == 'iopas') then
         call ocn_data_iopas
      endif
...
!=======================================================================
      subroutine nc_3d_time_maxrec(data_file, fieldname, timelen)
      ...
      character(len=*), parameter :: subname = '(nc_3d_time_maxrec)'

#ifdef USE_NETCDF
      integer (kind=int_kind) :: &
        status, &     ! status variable from netCDF routine
        varid, &      ! variable ID
        dimids(3)     ! dimensions ID
      ...
      if (my_task == master_task) then
          ...
          if (status /= nf90_noerr) then
             call abort_ice (subname//'ERROR: Inquiring dimids ' )
          endif

          status = nf90_inquire_dimension(fid, dimids(3), len = timelen)
          if (status /= nf90_noerr) then
             call abort_ice (subname//'ERROR: Getting timelen ' )
          endif
      ...
      end subroutine nc_3d_time_maxrec
...
!=======================================================================
      ...
      subroutine read_data_iopas_nc (flag, recd, yr, ixm, ixx, &
...
! Adapted by Alison McLaren, Met Office from read_data
      ...
      integer (kind=int_kind), intent(in) :: &
         recd                , & ! baseline record number
         yr                  , & ! year of forcing data
         ixm, ixx                ! record numbers of 2 data values
                                 ! relative to recd
      ...
      character(len=*), parameter :: subname = '(read_data_iopas_nc)'
      ...
      !-----------------------------------------------------------------
         ...
         if (ixx == 1) then
            if (yr > fyear_init) then ! get data from previous year
               call file_year (data_file, yr-1)
               call nc_3d_time_maxrec(data_file, fieldname, nrec)
            else             ! yr = fyear_init, no prior data exists
               nrec = recd + ixx
            endif            ! yr > fyear_init
         else
...
!         end if
         ...
         ! always read ixx data from data file for current year
         call file_year (data_file, yr)
         if (ixx==1) call ice_open_nc (data_file, fid)
      ...
      end subroutine read_data_iopas_nc
...
! that filenames end with 'yyyy.dat' or 'yyyy.r' or 'yyyy.nc'.
      ...
      elseif (trim(atm_data_type) == 'JRA55_tx1') then ! netcdf
         ...
         tmpname = data_file
         write(data_file,'(a,i4.4,a)') tmpname(1:i), yr, '.nc'
      elseif (trim(atm_data_type) == 'iopas') then ! netcdf
         i = index(data_file,'.nc') - 5
         tmpname = data_file
         write(data_file,'(a,i4.4,a)') tmpname(1:i), yr, '.nc'
...
!=======================================================================
      ...
      subroutine iopas_files (yr)

      integer (kind=int_kind), intent(in) :: &
           yr                   ! current forcing year

      character(len=*), parameter :: subname = '(iopas_files)'
      ...
      flw_file = &
           trim(atm_data_dir)//'/baltic_lwrad_down_2016.nc'
      call file_year(flw_file,yr)

      rain_file = &
           trim(atm_data_dir)//'/baltic_rain_2016.nc'
      call file_year(rain_file,yr)

      uwind_file = &
           trim(atm_data_dir)//'/baltic_wind_2016.nc'
      call file_year(uwind_file,yr)

      tair_file = &
           trim(atm_data_dir)//'/baltic_Tair_2016.nc'
      call file_year(tair_file,yr)

      humid_file = &
           trim(atm_data_dir)//'/baltic_Qair_2016.nc'
      call file_year(humid_file,yr)

      rhoa_file = &
           trim(atm_data_dir)//'/baltic_Pair_2016.nc'
      ...
      end subroutine iopas_files

!=======================================================================


      subroutine iopas_data

      use ice_flux, only: fsw, flw, fsnow, Tair, uatm, vatm, rhoa, Qa
      use ice_calendar, only: use_leap_years

      integer (kind=int_kind) :: &
          ixm,ixx      , & ! record numbers for neighboring data
          recnum       , & ! record number
          maxrec       , & ! maximum record number
          infile_maxrec, & ! maximum record number read from file
          recslot      , & ! spline slot for current record
          dataloc          ! = 1 for data located in middle of time interval
                           ! = 2 for date located at end of time interval
      ...
      logical (kind=log_kind) :: read1, isleap

      character (char_len) :: &
            fieldname    ! field name in netcdf file

      character(len=*), parameter :: subname = '(iopas_data)'
    ...
    ! 1-hourly data
    !
    ! Assume that the 1-hourly value is located at the end of the
    !  1-hour period.  This is the convention for NCEP reanalysis data.
    !  E.g. record 1 gives conditions at 1 am GMT on 1 January.
    !-------------------------------------------------------------------

      dataloc = 2               ! data located at end of interval
      sec1hr = secday/c2/c12    ! seconds in 1 hour

      isleap = .false.
      if (use_leap_years) then
         if (mod(fyear,  4) == 0) isleap = .true.
         if (mod(fyear,100) == 0) isleap = .false.
         if (mod(fyear,400) == 0) isleap = .true.
      endif
      ...
      if (istep==1 .or. (oldfyear /= fyear .and. &
         fyear == fyear_init )) then
         fieldname='Tair'
         call nc_3d_time_maxrec(tair_file, fieldname, infile_maxrec)
         oldrecd = infile_maxrec-maxrec
      elseif (oldfyear /= fyear) then
         oldrecd = 0 
      endif
      oldfyear = fyear
      ...
      ! Read
      read1 = .false.
      if (istep==1 .or. oldrecnum /= recnum) read1 = .true.
      ...
      if (trim(atm_data_format) == 'nc') then
         ...
         fieldname='Vwind'
         call read_data_iopas_nc (read1, oldrecd, fyear, ixm, ixx, &
            uwind_file, fieldname, vatm_data, &
            field_loc_center, field_type_vector)
         fieldname='Pair'
         call read_data_iopas_nc (read1, oldrecd, fyear, ixm, ixx, &
            rhoa_file, fieldname, rhoa_data, &
            field_loc_center, field_type_scalar)
         fieldname='Qair'
         call read_data_iopas_nc (read1, oldrecd, fyear, ixm, ixx, &
         ...
         call abort_ice (error_message=subname//'binary atm_data_format unavailable', &
            file=__FILE__, line=__LINE__)
      endif
      ...
      Tair = Tair + Tffresh
      rhoa = 100 * rhoa / (Tair * 287.058_dbl_kind)

      ! Save record number for next time step
      oldrecnum = recnum

!      if (my_task == master_task) then
!         write (nu_diag,*) '************MACIEJ******************* '
!         write (nu_diag,*) 'Model date =',myear,mmonth,mday,msec
!         write (nu_diag,*) 'Tair file', trim(tair_file)
!         write (nu_diag,*) 'ixm,ixx,oldrecd',ixm,ixx,oldrecd
!         write (nu_diag,*) 'max fsw', maxval(fsw)
!         write (nu_diag,*) 'max flw', maxval(flw)
!         write (nu_diag,*) 'max Tair', maxval(Tair)
!         write (nu_diag,*) 'max uatm', maxval(uatm)
!         write (nu_diag,*) 'max vatm', maxval(vatm)
!         write (nu_diag,*) 'max fsnow', maxval(fsnow)
!         write (nu_diag,*) 'max rhoa', maxval(rhoa)
!         write (nu_diag,*) 'max Qa', maxval(Qa)
!         write (nu_diag,*) '************MACIEJ_END******************* '
!      endif                     ! master_task

      end subroutine iopas_data
!=======================================================================
! NCAR atmospheric forcing
!=======================================================================
...
!=======================================================================
      ...
      subroutine ocn_data_iopas_init
        ...
        character(len=*), parameter :: subname = '(ocn_data_iopas_init)'
        ...
        sst_file = trim(ocn_data_dir)//'/ocean_2000.nc'
        call file_year(sst_file,fyear)

        if (my_task == master_task) then
         write (nu_diag,*) ' '
         write (nu_diag,*) 'Forcing data year =', fyear
         write (nu_diag,*) 'Oceanic data files:'
         write (nu_diag,*) trim(sst_file)
        endif

      end subroutine ocn_data_iopas_init

!=======================================================================

      subroutine ocn_data_iopas

        use ice_calendar, only: use_leap_years
        use ice_blocks, only: nx_block, ny_block, block, get_block
        use ice_domain_size, only: max_blocks
        use ice_flux, only: ss_tltx, ss_tlty, frzmlt, sst, sss, uocn, vocn
        use ice_boundary, only: ice_HaloUpdate
        ...
        integer (kind=int_kind) :: &
            i, j, iblk   , & ! horizontal indices
            ilo,ihi,jlo,jhi, &   ! beginning and end of physical domain
            ixm,ixx,ixp  , & ! record numbers for neighboring data
            recnum       , & ! record number
            modadj       , & ! adjustment for mod function
            maxrec       , & ! maximum record number
            recslot      , & ! spline slot for current record
            dataloc          ! = 1 for data located in middle of time interval
                             ! = 2 for date located at end of time interval
        ...
        character(len=*), parameter :: subname = '(ocn_data_iopas)'
        ...
        if (ixx.eq.1) then
          if (use_leap_years) then
            if (mod(fyear-1,  4) == 0) isleap = .true.
            if (mod(fyear-1,100) == 0) isleap = .false.
            if (mod(fyear-1,400) == 0) isleap = .true.
          endif
          if (isleap) then
            ixm = 8784
          else
        ...
        read1 = .false.
        if ( mod(msec,3600).eq.0 ) read1 = .true.

        if (trim(atm_data_format) == 'nc') then
          fieldname = 'sst'
          call read_data_nc (read1, 0, fyear, ixm, ixx, ixp, &
                             maxrec, sst_file, fieldname, sst_data, &
                             field_loc_center, field_type_scalar)  
          fieldname = 'sss'
          call read_data_nc (read1, 0, fyear, ixm, ixx, ixp, &
                             maxrec, sst_file, fieldname, sss_data, &
                             field_loc_center, field_type_scalar)  
          fieldname = 'uocn'
          call read_data_nc (read1, 0, fyear, ixm, ixx, ixp, &
                             maxrec, sst_file, fieldname, uocn_data, &
                             field_loc_center, field_type_vector)  
          fieldname = 'vocn'
          call read_data_nc (read1, 0, fyear, ixm, ixx, ixp, &
                             maxrec, sst_file, fieldname, vocn_data, &
                             field_loc_center, field_type_vector)  
          fieldname = 'zeta'
          call read_data_nc (read1, 0, fyear, ixm, ixx, ixp, &
                             maxrec, sst_file, fieldname, zeta_data, &
                             field_loc_center, field_type_scalar)  
          fieldname = 'frzmlt'
          call read_data_nc (read1, 0, fyear, ixm, ixx, ixp, &
                             maxrec, sst_file, fieldname, frzmlt_data, &
                             field_loc_center, field_type_scalar)  
        else
          call abort_ice (error_message=subname//'binary atm_data_format unavailable', &
            file=__FILE__, line=__LINE__)
        ...
        call interpolate_data (zeta_data,work1)
        ...
        do iblk = 1, nblocks
          ...
          do j = jlo, jhi
             do i = ilo, ihi
                ss_tltx(i,j,iblk) =                                  &
                     p5*((work1(i+1,j,iblk) + work1(i+1,j+1,iblk)) -   &
                     (work1(i,j,iblk) + work1(i,j+1,iblk))) /dxu(i,j,iblk)
                ss_tlty(i,j,iblk) =                                  &
                     p5*((work1(i,j+1,iblk) + work1(i+1,j+1,iblk)) -   &
                     (work1(i,j,iblk) + work1(i+1,j,iblk))) /dyu(i,j,iblk)
             enddo
          enddo
        enddo
        ...
        call interpolate_data (frzmlt_data,frzmlt)
        call ice_HaloUpdate (frzmlt, halo_info, &
            field_loc_center, field_type_scalar)
         
      end subroutine ocn_data_iopas      
