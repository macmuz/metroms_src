...
! 2007: ECH added dynamic file units, modified from POP_IOUnitsMod.F90
      ...
      integer (kind=int_kind), parameter, public :: &
         ...
         ice_stderr =  510    ! reserved unit for standard error
...
#ifdef ROMSCOUPLED
      integer (kind=int_kind), public :: &
         ice_IOUnitsMinUnit = 511, & ! do not use unit numbers below 
         ice_IOUnitsMaxUnit = 599    ! or above, set by setup_nml
#else
      integer (kind=int_kind), public :: &
...
!  nu_nml is obtained separately.
      ...
      subroutine init_fileunits
         ...
         write(filename,'(a,".",a)') 'cice_nu_diag',trim(sysdate)
         open(UNIT=nu_diag,FILE=trim(filename))
         write(filename,'(a,".",a)') trim(stdout_file),trim(sysdate)
         open(UNIT=ice_stdout,FILE=trim(filename))
         if (ice_stdout.ne.ice_stderr) then
            write(filename,'(a,".",a)') 'cice_stderr',trim(sysdate)
            open(UNIT=ice_stderr,FILE=trim(filename))
         endif
#else
         ice_IOUnitsInUse = .false.
