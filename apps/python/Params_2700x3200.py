...
class Params(object):
    ...
    def __init__(self,app,xcpu,ycpu,start_date,end_date,nrrec=0,cicecpu=0,restart=False,copy=True):
        ...
        if app=='baltic':
            ...
            self.DELTAT=120
            self.CICEDELTAT=360
            self.COUPLINGTIME_I2O=360
            self.ROMSINFOLDER="/users/work/mmuzyka/CSDIR/input_2700x3200"
            self.ROMSINIFILE = self.ROMSINFOLDER+"/ROMS_initial_20160701.nc"
            if restart == True:
            ...
            ########################################################################
            self.KEYWORDLIST=[
            ['APPTITLE',"ROMS 1087 - baltic 0.25 NM - Coupled ROMS-CICE"],
            ['MYAPPCPPNAME',"BALTIC"],
            ['VARFILE',GlobalParams.tmpdir+"/roms_src/ROMS/External/varinfo.dat"],
            ['XPOINTS',"2698"],  #Could read from grd-file?
            ['YPOINTS',"3198"],  #Could read from grd-file?
            ...
            ['HISSTEPP',str(6*3600/int(self.DELTAT))],
            ...
            ['OBCFAKTOR',"0.0"],
            ['NUDGZONEWIDTH',"10"],
            ['GRDFILE',self.ROMSINFOLDER+"/ROMS_grid.nc"],
            ['INIFILE',self.ROMSINIRST],
            ['RUNDIR',self.RUNPATH],
            ['RIVERFILE',self.ROMSINFOLDER+"/ROMS_rivers.nc"],
            ['_BRYNAME_',self.ROMSINFOLDER+"/ROMS_bc_2016.nc"],
